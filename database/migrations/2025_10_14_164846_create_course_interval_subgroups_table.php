...
return new class extends Migration
{
    ...
    {
        $groupIdUnsigned = $this->columnIsUnsigned('course_interval_groups', 'id');
        $subgroupIdUnsigned = $this->columnIsUnsigned('course_subgroups', 'id');

        if (!Schema::hasTable('course_interval_subgroups')) {
            Schema::create('course_interval_subgroups', function (Blueprint $table) use ($groupIdUnsigned, $subgroupIdUnsigned) {
                $table->id();
                $groupIdUnsigned
                    ? $table->unsignedBigInteger('course_interval_group_id')
                    : $table->bigInteger('course_interval_group_id');
                $subgroupIdUnsigned
                    ? $table->unsignedBigInteger('course_subgroup_id')
        ...
        $this->ensureBigIntegerColumn('course_interval_subgroups', 'course_interval_group_id', $groupIdUnsigned);
        $this->ensureBigIntegerColumn('course_interval_subgroups', 'course_subgroup_id', $subgroupIdUnsigned);

        $this->ensureForeignKey(
            'course_interval_subgroups',
            'course_interval_subgroups_course_interval_group_id_foreign',
        ...
        $this->ensureForeignKey(
            'course_interval_subgroups',
            'course_interval_subgroups_course_subgroup_id_foreign',
            'course_subgroup_id',
            'course_subgroups',
            'id'
        );
