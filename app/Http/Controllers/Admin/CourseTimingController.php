...
class CourseTimingController extends Controller
{
    ...
    {
        try {
            $query = BookingUserTime::with(['bookingUser.client', 'courseDate']);

            // Apply filters
            if ($request->has('course_id')) {
                $query->whereHas('courseDate', function ($q) use ($request) {
                    $q->where('course_id', $request->course_id);
                });
            }

            if ($request->has('course_date_id')) {
                $query->where('booking_user_times.course_date_id', $request->course_date_id);
            }
            ...
            // Transform data to include student_id directly
